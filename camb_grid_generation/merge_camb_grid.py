...
#What gets saved is not the raw grid but its cubic B-spline *coefficients*, together with
#the knot vectors: that pair is exactly what scipy.interpolate.NdBSpline consumes, so
#cmb_lensing.camb_grid_interp evaluates the merged file directly with no per-call setup.
#Handing the raw values to RegularGridInterpolator(method="cubic") instead is not an
#option: it re-solves for these same coefficients inside its constructor, at a cost linear
#in the ell axis (of order ten minutes per spectrum at 3998 ells), and it solves with an
#iterative Krylov solver rather than the direct banded solve make_interp_spline does
#below, landing ~4e-4 away in lnCl - the size of the whole error budget.
#
#Usage:
