...
#
#To DROP the flatness assumption instead, set derivative_params below. The script then runs the
#reference cosmology into $out_dir/reference and each listed parameter at +/- derivative_sigma
#into $out_dir/<param>_plus and $out_dir/<param>_minus, all on the SAME seeds (common random
#numbers, which is what cancels the Monte Carlo noise in the difference), and
#    python merge_delensed_spectra.py --spectra_dir "$out_dir/reference" \
#        --shifted_dirs "$out_dir"/*_plus "$out_dir"/*_minus
#writes R_0 AND dR/dtheta into one transfer_function.npz, which fisher_forecast's
#--transfer_function then evaluates at every stencil point. (2k + 1) x num_realizations jobs
...
#which cosmology map_joint's covariances and the CAMB reference's Alens_L are built at when
#the data are shifted: 1 holds them at GROUND_TRUTH (the frozen estimator the forecast's
#stencil assumes, and the only convention the dR/dtheta merge accepts); 0 rebuilds them at the
#shifted cosmology, which is how runs before 2026-09-18 were made
freeze_reconstruction=1

#dR/dtheta mode: a space-separated list of PARAM_ORDER names (e.g. "omch2 theta_MC_100 logA").
#Leave empty for the single run above - when set, shift_param / shift_value are ignored.
#derivative_sigma is the displacement in units of PARAM_SIGMA. It is NOT the forecast's 0.05
#sigma step: the common random numbers keep the difference quiet at any Delta, so make it wide
#enough that the signal clears the residual noise, and narrow enough that R is still linear -
#the merge prints the second-order term as a check
derivative_params=""
derivative_sigma=0.5
...
if [ "$freeze_reconstruction" = "1" ]; then
    reconstruction=fiducial
else
    reconstruction=shifted
fi

#one full set of realizations into one directory. Every set uses the same seeds - within a
#set load_transfer_directory refuses duplicates, so a loop change that repeats one fails the
#merge loudly instead of double counting; across sets the SHARED seeds are the common random
#numbers the flatness test and the dR/dtheta merge both pair on
submit_set () {
    local dir="$1" param="$2" value="$3" unit="$4"
    mkdir -p "$dir"
    for ((m=0; m<num_realizations; m++)); do
        ...
        sbatch get_single_delensed_spectra.sh "$m" "$map_seed" "$nside" "$theta_pix" \
            ...
            "$qe_response" "$dir" "$param" "$value" "$unit" "$reconstruction"
    done
}

if [ -z "$derivative_params" ]; then
    submit_set "$out_dir" "$shift_param" "$shift_value" value
else
    #the dR/dtheta layout merge_delensed_spectra.py --shifted_dirs expects: one reference
    #set and a plus / minus set per parameter, all under out_dir
    submit_set "$out_dir/reference" "" 0 value
    for param in $derivative_params; do
        submit_set "$out_dir/${param}_plus" "$param" "$derivative_sigma" sigma
        submit_set "$out_dir/${param}_minus" "$param" "-$derivative_sigma" sigma
    done
fi
