...
def main():
    ...
    if worst_drift <= args.tolerance:
        ...
        print(f"  VERDICT: R drifts {worst_drift:.2e} over one finite-difference step, "
              ...
              f"--transfer_function; if it is a real fraction of the signal, the transfer "
              f"function must be measured per stencil point with common random numbers "
              f"(same seeds at theta and theta +/- h) instead of held fixed")
