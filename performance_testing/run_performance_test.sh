...
for pol_key in "IP"; do # "I" "P"; do
    #loop over map size values
    for map_size in 128 256; do # 256 512 1024; do
        ...
        for ((seed=1; seed<=num_seeds; seed++)); do
            ...
            for ((trial=1; trial<=num_trials; trial++)); do
                ...
                sbatch julia_performance_test.sh $map_size $seed $trial $pol_key
                sbatch python_performance_test.sh $map_size $seed $trial $pol_key
            done
        done
    done
done