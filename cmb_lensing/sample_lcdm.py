...
def prepare_emulator_jax(emulator):
    ...
    return predict_tt, predict_pp, emu_params
...
#@jax.jit
def gibbs_sample_theta(theta_key_idx, theta_old, theta_range, mixed_temp, mixed_phi,
                       data_field, args, current_params, predict_tt, predict_pp,
                       emu_params, rng_key, over_relaxation_num_samps = -1):
    """Sample a single cosmological parameter via grid evaluation + inverse CDF.

    theta_key_idx: integer index into PARAM_ORDER for the parameter being sampled.
    current_params: jnp array of shape (5,) with current values in PARAM_ORDER.
    predict_tt, predict_pp: pure-JAX emulator functions from prepare_emulator_jax.
    emu_params: Flax weight pytree from prepare_emulator_jax.
    """
    lmax = args["lmax"]
    lmax_prime = args["lmax_prime"]
    ...
    def single_logpdf(i):
        ...
        g = get_g_matrix(cphi, args["quadratic_estimate"].scalar_matrix, 1, a_phi = 1)
        d = get_d_tt_matrix(cf, jnp.zeros_like(cf),
                            args["noise_covariance"].scalar_matrix, 1, 1)

        phi_cov = args["phi_covariance"].replace(scalar_matrix = cphi)
        field_cov = args["field_covariance"].replace(scalar_matrix = cf)
        mixing_g = args["mixing_g"].replace(scalar_matrix = g)
        mixing_d = args["mixing_d"].replace(scalar_matrix = d)

        return mixed_logpdf(mixed_temp, mixed_phi, data_field,
                            args["noise_covariance"], phi_cov, field_cov,
                            args["mask"], args["beam"], mixing_g, mixing_d)

    logpdf_values = jax.vmap(single_logpdf)(jnp.arange(N))

    jax.debug.print("LogPDF Values for {param} = {values}", param = theta_key_idx, values = logpdf_values)
...
def grid_and_sample(logpdf_values, theta_values, sub_key, theta_old,
    ...
    jax.debug.print("LogPDF Values = {vals}", vals = logpdf_values)
...
#fast replacement to CAMB to calculate cls for a given cosmology
def emulate_cls(theta_MC_100, logA, ns, ombh2, omch2, emulator, lmax, lmax_prime):
        
        cls = emulator.predict({"theta_MC_100": theta_MC_100, "logA": logA,
                                "ns": ns, "ombh2": ombh2, "omch2": omch2})
        cl_tt = interpolate_cls(cls["tt_unlensed"], lmax, lmax_prime)
        cl_pp = interpolate_cls(cls["pp"], lmax, lmax_prime)

        return cl_tt, cl_pp

#fast method to compute the matrices that change as a function of the
#cosmological parameters (i.e. Cf, Cphi, D, and G)
def get_new_cosmo_matrices(theta_MC_100, logA, ns, ombh2, omch2, emulator, args):

    cl_tt, cl_pp = emulate_cls(theta_MC_100, logA, ns, ombh2, omch2, emulator, 
...
def jax_get_new_cosmo_matrices(current_params, predict_tt, predict_pp, emu_params, args):
...
def sample_joint(data_set, param_init, param_ranges, should_sample, iters_per_chain = 500,
    ...
    args = {}
    args["lmax"] = lmax
    args["lmax_prime"] = min(lmax, 4000) #This is the default length of Yuuki's emulator's ell-range
    ...
    ells = jnp.arange(2, lmax + 1).astype(jnp.float64)
    args["ells"] = ells
    ...
    predict_tt, predict_pp, emu_params = prepare_emulator_jax(emulator)
    ...
    #We should therefore initialize the QE norm to be computed based on our initial parameter
    #guesses...
    ...
    initial_set = load_sim(nside, theta_pix, pol, master_seed, **param_init,
    ...
    qe_matrix = scalar_quadratic_estimate(args["noise_covariance"].scalar_matrix, 
                                          ...
                                          initial_set.lensed_field_covariance.scalar_matrix, 
    ...
    #We must initialize the mixing D & G matrices as well as Cf and Cphi
    #to the proper values according to our starting guess for the cosmological parameters
    g, d, _, cphi = jax_get_new_cosmo_matrices(current_params, predict_tt, predict_pp,
                                                 emu_params, args)
    ...
    for iter in range(1, iters_per_chain):
        ...
        if iter >= num_burn_in_fix_theta:
            for theta, theta_range in param_ranges.items():
                if should_sample[theta]:
                    ...
                    theta_val = gibbs_sample_theta(theta_key_idx, param_vals[theta][-1],
                                                   theta_range, mixed_temp, mixed_phi,
                                                   data_field, args, current_params,
                                                   predict_tt, predict_pp, emu_params, rng_key,
        ...
        g, d, cf, cphi = jax_get_new_cosmo_matrices(current_params, predict_tt, predict_pp,
                                                     emu_params, args)
...
if __name__ == "__main__":
    ...
    #try and learn the cosmological parameter distributions
    start_time = time.time()

    #initial starting guesses for parameters (fiducial values)
    ground_truth_params = {}
    ground_truth_params["ombh2"] = 0.0224
    ground_truth_params["omch2"] = 0.109
    ground_truth_params["cosmomc_theta"] = 0.010411
    ground_truth_params["As"] = jnp.exp(3.05) * 1e-10
    ground_truth_params["ns"] = 0.965
    ...
    param_init = {}
    param_init["ombh2"] = 0.05
    param_init["omch2"] = ground_truth_params["omch2"]
    param_init["theta_MC_100"] = ground_truth_params["cosmomc_theta"] * 100
    param_init["logA"] = jnp.log(ground_truth_params["As"] * 1e10)
    param_init["ns"] = ground_truth_params["ns"]

    #TODO consult with others on allowed search ranges...
    #allowed search / sample range for parameters... I cannot do this unless
    #Yuuki shares their training data or parameter specifications
    SEARCH_PRECISION = 200
    param_ranges = {}
    param_ranges["ombh2"] = jnp.linspace(0.005, 0.05, SEARCH_PRECISION)
    param_ranges["omch2"] = jnp.linspace(0.001, 0.99, SEARCH_PRECISION)
    param_ranges["theta_MC_100"] = jnp.linspace(0.5, 2.5, SEARCH_PRECISION)
    param_ranges["logA"] = jnp.linspace(2.0, 4.0, SEARCH_PRECISION)
    param_ranges["ns"] = jnp.linspace(0.8, 1.2, SEARCH_PRECISION)
